...
class InelsClimate(InelsBaseEntity, ClimateEntity):
    ...
    def __init__(
        ...
        self.entity_id = (
            f"{Platform.CLIMATE}.{self._attr_unique_id}"
        )

        self._attr_name = (
            f"{self._attr_name} {description.name}"
        )
    ...
    def target_temperature_high(self) -> float | None:
        """Get target high temperature."""

        return self._device.state.__dict__[self.key].required

    @property
    def target_temperature_low(self) -> float | None:
        """Get target low temperature."""
    ...
    def hvac_modes(self) -> list[HVACMode] | list[str]:
        ...
        if hasattr(val, "control_mode"):
            if val.control_mode == 0:
                return [
                    HVACMode.OFF,
                    HVACMode.HEAT,
                    HVACMode.COOL,
    ...
    def hvac_mode(self) -> HVACMode | str | None:
        ...
        return CLIMATE_MODE_TO_HVAC_MODE.get(
            val.climate_mode
        )
    ...
    def hvac_action(self) -> HVACAction | str | None:
        ...
        return CLIMATE_ACTION_TO_HVAC_ACTION.get(
            val.current_action
        )
    ...
    def preset_mode(self) -> str | None:
        ...
        if (
            hasattr(val, "current_preset")
            and val.control_mode == 0
        ):
    ...
    async def async_set_hvac_mode(
        ...
        if hasattr(
            ...
            if (
                ...
                ha_val.__dict__[
                    self.key
                ].required = last_val.__dict__[
                    self.key
                ].required

                ha_val.__dict__[
                    self.key
                ].required_cool = last_val.__dict__[
                    self.key
                ].required_cool
    ...
    async def async_set_preset_mode(
        ...
        new_preset = (
            self.entity_description.presets.index(
                preset_mode
            )
        )
